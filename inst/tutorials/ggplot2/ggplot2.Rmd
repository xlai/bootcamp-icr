...
The third thing to stress is that calls to `ggplot` and followed by calls to any number of _geometries_.
...
You can use any number of geometries in a plot.
...
In this tutorial we will learn how to reflect group-level information in `ggplot2` using:
...
If your plot is a bit too noisy, sometimes a very useful trick is to make several versions of the plot, with each showing the data for one particular group.
...
Edit the example above to produce a plot with two columns:
...
A common example of this in medical research is repeated outcome measures.
...
but this plot shows no grouping information: we do not know which points join up.
Edit this example by appending `geom_line()` to get line-plots:
...
We need to set the `group` aesthetic within the call to `geom_line` to be `TNO` to reflect that only points within the same patient should be joined.
Do that now:

```{r tumour-lines-grouped, exercise = TRUE, exercise.lines = 5}
tumours %>% 
  ggplot(aes(x = Time, y = TumourSize)) + 
  geom_point()
```

We will finish off this example by producing a plot that is commony seen in cancer trials literature.
A_spider plot_ shows the repeated measures of tumour size _relative to baseline_.
This last clause is important because tumour sizes are analysed under the RECIST criteria for response compared to baseline.
...
Notice that the `TumourSize` variable in this dataset has been given the alias `Baseline`.
...
Note:
The significance of the left in `left_join` is that the LHS record is retained even if no match in the RHS could be found.
...
Do that now using the `mutate` command:
