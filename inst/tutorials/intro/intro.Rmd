...
```{r vectorise2, exercise=TRUE, exercise.eval=TRUE}
abc + abc^2
```
...
You can access the elements of a `vector` using square brackets and the intger index.
The first elelment is:
```{r}
abc[1]
```
...
Wheres vectors contained multiple instances of the same type, lists can contain values of different types.
For example:
...
You access elements of a list using double square brackets and either integer indices:
```{r}
l[[1]]
```
...
```{r}
trials <- data.frame(
  name = c('PePS2', 'BTOG2', 'Viola', 'RomiCar', 'Matrix', 'PIIIa'),
  disease = c('lung', 'lung', 'haem', 'haem', 'lung', 'dunno'),
  n = c(62, 1363, 29, 20, 200, 600),
  arms = c(1, 3, 1, 1, 20, 2),
...
For example, let's say we are interested in calculating the average sample size by trial phase.
...
### Desriable changes
There are, however, some things we request that RStudio users do in CRCTU to promote reproducible research and coding for others.
...
Historically, this has created problems in CRCTU so we advise that you change that manually.
The following steps can be run on your CRCTU machine to locate the R library to the C drive.
...
Exercise: what packages does the R package`lme4` rely on?
Look up the CRAN page for `lme4`.
...
What function(s) does the package provide?
...
However, to promote reproducility, the historic releases are available for manual download as a zip file from the CRAN page.
After download, these can be installed manually.

TODO - install `crctu` package from zip.
